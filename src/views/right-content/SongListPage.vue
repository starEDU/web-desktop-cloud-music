...
export default {
  ...
  setup(){
    ...
    const songListPageState = reactive({
      ...
      //播放全部
      playAll(index){
	      console.log(songListPageState['playlist']['tracks'])
	      commit('setPlaySongList',songListPageState['playlist']['tracks'])
	      commit('setCurrentPlayIndex',index)
	      musicPlayTool(songListPageState['playlist']['tracks'],index)
      },
      // 收藏或取消收藏
      async subscribe(t){
